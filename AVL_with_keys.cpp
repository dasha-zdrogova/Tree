...
void print_all(Node* root, int offset = 0) {
    if (!root) return;
    print_all(root->right, offset+3);
    for (int i = 0; i < offset; ++i) cout << " ";
    cout << root->key << "\n";
    print_all(root->left, offset+3);
}
...
Node* del(Node* root, int key){
    ...
    // if (balance > 1) {
    //     if (get_balance(root->left) < 0) root->left = left_rotate(root->left);
    //     return right_rotate(root);
    // }
    // if (balance < -1) {
    //     if (get_balance(root->right) > 0) root->right = right_rotate(root->right);
